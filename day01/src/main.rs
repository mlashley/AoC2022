...
fn main() {
    ...
    if let Ok(lines) = read_lines("./input.txt") {
            ...
            if let Ok(cal) = line {
                ...
                if i == 0 {
                    ...
                    if current_elf_sum > elf_max  {
                        ...
                        println!("Debug: Elf {} is new max {}",elf_carrying_vector.len(),current_elf_sum);
                    }
                    current_elf_sum=0;
                }
            }
        ...
        if current_elf_sum > elf_max  {
            elf_max = current_elf_sum;
            println!("Elf {} is new max {}",elf_carrying_vector.len(),current_elf_sum);
        }

        let elf_count = elf_carrying_vector.len();
        println!("Part1: Processed {} elves and max is carrying {} calories",elf_count,elf_max);
        elf_carrying_vector.sort();
        let top_three = elf_carrying_vector.split_off(elf_count-3);
        println!("Part2: Top 3 elves {:?}",top_three);
        println!("Part2: Top 3 total {} calories", top_three.into_iter().sum::<i32>());
    }
}
